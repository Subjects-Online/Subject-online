...
function createRamadanHeader() {
    const header = document.createElement('div');
    header.className = 'ramadan-header';
    
    const content = document.createElement('div');
    content.className = 'ramadan-header-content';
    content.innerHTML = `
        <span class="ramadan-header-icon">🌙</span>
        <span class="ramadan-header-text">رمضان كريم - كل عام وأنتم بخير</span>
        <span class="ramadan-header-icon">✨</span>
    `;
...
function createRamadanSparkles() {
...
function addRamadanEffects() {
    ...
    setInterval(addTimeBasedEffects, 60000);
}

function addTimeBasedEffects() {
    const hour = new Date().getHours();
    const header = document.querySelector('.ramadan-header');
    const moon = document.querySelector('.ramadan-moon');
    
    if (header) {
        if (hour >= 5 && hour < 12) {
            // Morning - softer glow
            header.style.filter = 'brightness(0.9)';
        } else if (hour >= 12 && hour < 17) {
            // Afternoon - normal
            header.style.filter = 'brightness(1)';
        } else if (hour >= 17 && hour < 20) {
            // Iftar time - enhanced glow
...
// Create floating particles every 8 seconds
setInterval(createFloatingParticle, 8000);
