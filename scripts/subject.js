...
(function () {
  ...
  tabBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (subjectId)
        sessionStorage.setItem("so_tab_" + subjectId, btn.dataset.tab);
    });
  });
  ...
  function buildAccordion(chapter, panelKey) {
    ...
    const cbId =
      chapter.id || `ch-${panelKey}-${Math.random().toString(36).slice(2, 7)}`;
  ...
  function attachLectureClicks() {
    document.querySelectorAll(".lecture-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        if (
          e.target.classList.contains("progress-check") ||
          e.target.classList.contains("lecture-check")
        )
          return;
        ...
        if (!url) {
          showToast(
            "لسه معدتش ملف لـ lecture ده — افتح Admin Panel عشان تضيفه",
            "info",
          );
        } else if (isVideo) {
          openVideo(url, title);
  ...
  function autoExpandFirst() {
    document
      .querySelectorAll(
        ".tab-panel.active .chapter-accordion:first-child .chapter-header",
      )
  ...
  window.openLecturePDF = function (url, title, meta) {
    ...
    subEl.textContent =
      document.querySelector(".subject-hero h1")?.textContent || "Subject";
